...
func NewReader(config config.SupplierConfig, library *terraform.ProviderLibrary, backendOpts *backend.Options, resourceSchemaRepository *resource.SchemaRepository) (*TerraformStateReader, error) {
	...
	for _, deserializer := range r.deserializers {
		...
		for _, res := range decodedResources {
			...
			if resource.IsRefactoredResource(res.TerraformType()) {
				...
				ctyAttr := rescty.ToCtyAttributes(res.CtyValue())
